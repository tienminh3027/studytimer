...
function renderCountdowns() {
  ...
  if (sorted.length === 0) {
    dom.countdownList.innerHTML = '<p class="empty-state">Chưa có ngày đếm ngược nào. Thêm một mốc quan trọng để theo dõi!</p>';
    return;
  }
...
function cacheDom() {
  const ids = [
    ...
    'countdownCard', 'countdownTitleInput', 'countdownDateInput', 'countdownAddBtn', 'countdownList',
    'logList', 'btnClearAll',
...
function wireEvents() {
  ...
  dom.countdownAddBtn.addEventListener('click', addCountdown);
